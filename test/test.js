...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterLogit, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterLogit( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the logit function for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the logit function for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': logit( values[ 0 ] ),
			'done': false
		},
		{
			'value': logit( values[ 1 ] ),
			'done': false
		},
		{
			'value': logit( values[ 2 ] ),
			'done': false
		},
		{
			'value': logit( values[ 3 ] ),
			'done': true
		}
	];
